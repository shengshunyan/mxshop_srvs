...
func (g *GoodsServer) GoodsList(ctx context.Context, req *proto.GoodsFilterRequest) (*proto.GoodsListResponse, error) {
	...
	result := localDB.Preload("Category").Preload("Brands").Scopes(utils.Paginate(int(req.Pages), int(req.PagePerNums))).Find(&goods)
	if result.Error != nil {
		return nil, result.Error
	}
	goodsListResponse.Data = slice.Map(goods, ModelToResponse)
...
func (g *GoodsServer) GetGoodsDetail(ctx context.Context, req *proto.GoodInfoRequest) (*proto.GoodsInfoResponse, error) {
	...
	result := global.DB.Where(req.Id).Find(&good)
	if result.Error != nil {
		return nil, result.Error
	}
